        ...
        <form @submit.prevent="storeData">
          ...
          <div class="form-grid">
            <div class="form-group">
              <label>NIM</label>
              <input type="text" v-model="form.nim" placeholder="Contoh: 26000123" required />
              <small v-if="errors.nim" class="text-danger">{{ errors.nim[0] }}</small>
            </div>

            <div class="form-group">
              <label>Nama Lengkap</label>
              <input type="text" v-model="form.nama" placeholder="Nama Mahasiswa" required />
              <small v-if="errors.nama" class="text-danger">{{ errors.nama[0] }}</small>
            </div>

            <div class="form-group">
              <label>Email Kampus</label>
              <input type="email" v-model="form.email" placeholder="nama@binus.ac.id" required />
              <small v-if="errors.email" class="text-danger">{{ errors.email[0] }}</small>
            </div>

            <div class="form-group">
              <label>Angkatan</label>
              <input type="number" v-model="form.angkatan" placeholder="2025" required />
...
export default {
  data() {
    return {
      ...
      errors: {}, // Menampung error validasi per field
      errorMessage: '',
      isLoading: false
    }
  },
  methods: {
    async storeData() {
      ...
      try {
        ...
        // Kirim ke Backend
        await axios.post('http://127.0.0.1:8000/api/mahasiswa', this.form, {
        ...
        if (error.response && error.response.status === 422) {
            // Error Validasi (Misal NIM kembar)
...
.alert-error { background: #ffebee; color: #c62828; padding: 10px; border-radius: 8px; margin-bottom: 20px; }

.btn-back { background: transparent; border: 1px solid #ddd; padding: 8px 15px; border-radius: 5px; cursor: pointer; }
.btn-submit {
  width: 100%; padding: 14px; background: linear-gradient(90deg, #d500f9, #bf00ff);
  color: white; border: none; border-radius: 8px; font-weight: bold; cursor: pointer; margin-top: 20px;
}
